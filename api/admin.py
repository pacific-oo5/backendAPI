...
class VacancyAdmin(admin.ModelAdmin):
    list_display = ('name', 'user_email', 'work_type', 'salary', 'country_city', 'is_active', 'published_at')
    list_filter = ('work_type', 'work_time', 'is_active', 'is_remote', 'published_at')
    search_fields = ('name', 'description', 'user__email')
    readonly_fields = ('published_at',)
    fieldsets = (
        ('Основная информация', {
            'fields': ('user', 'name', 'description', 'about_me', 'salary')
        }),
        ('Детали вакансии', {
...
class AnketaAdmin(admin.ModelAdmin):
    list_display = ('name', 'user_email', 'country_city', 'phone_number', 'is_active', 'created_at')
    list_filter = ('is_active', 'created_at')
    search_fields = ('name', 'about_me', 'user__email')
    readonly_fields = ('created_at', 'user_profile_link')
    fieldsets = (
        ('Основная информация', {
            'fields': ('user', 'user_profile_link', 'name', 'about_me')
        }),
        ('Опыт работы', {
            'fields': ('experience',),
            'classes': ('collapse',)
